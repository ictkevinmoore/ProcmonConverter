...
if (Test-Path $mainScript) {
    Write-Host "  [PASS] Main script found: $mainScript" -ForegroundColor Green
} else {
...
if (Test-Path $reportScript) {
    Write-Host "  [PASS] Report generator found: $reportScript" -ForegroundColor Green
} else {
...
if (Test-Path $processorScript) {
    Write-Host "  [PASS] Streaming processor found: $processorScript" -ForegroundColor Green
} else {
    Write-Host "  [FAIL] Streaming processor NOT found" -ForegroundColor Red
    exit 1
}
...
if (Test-Path $sampleDataDir -PathType Container) {
    Write-Host "  [PASS] Sample data directory found: $sampleDataDir" -ForegroundColor Green

    $csvFiles = Get-ChildItem -Path $sampleDataDir -Filter "*.csv" -File
    if ($csvFiles.Count -gt 0) {
        Write-Host "  [PASS] Found $($csvFiles.Count) CSV file(s)" -ForegroundColor Green
    } else {
        Write-Host "  [WARN] No CSV files found in sample data" -ForegroundColor Yellow
    }
} else {
    Write-Host "  [FAIL] Sample data directory NOT found" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "  [PASS] Dot-sourcing with quotes successful!" -ForegroundColor Green

    # Check if function is available
    if (Get-Command "New-ProfessionalReport" -ErrorAction SilentlyContinue) {
        Write-Host "  [PASS] New-ProfessionalReport function loaded" -ForegroundColor Green
    } else {
        Write-Host "  [WARN] New-ProfessionalReport function not available" -ForegroundColor Yellow
    }
} catch {
    Write-Host "  [FAIL] Dot-sourcing failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "`n  [PASS] Script execution completed successfully!" -ForegroundColor Green
} catch {
    Write-Host "`n  [FAIL] Script execution failed: $($_.Exception.Message)" -ForegroundColor Red
