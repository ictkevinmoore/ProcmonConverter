...
class AdvancedAnalyticsEngine {
    ...
    [AnalyticsResult] AnalyzeData([hashtable]$processedData) {
        ...
        # ENHANCEMENT: Data integrity validation before caching and return
        $this.ValidateAnalyticsResult($result)
    ...
    [hashtable] CalculateMetricsOptimized([hashtable]$processedData) {
        ...
        if ($stats.ContainsKey('Results') -and $stats.Results) {
            ...
            # OPTIMIZATION: Single enumeration with conditional logic using compiled regex
            foreach ($key in $results.Keys) {
                $count = $results[$key]
                $totalResults += $count

                # PERFORMANCE: Use compiled regex instead of inline -notmatch
                if (-not $this.SuccessRegex.IsMatch($key)) {
                    $errorsCount += $count
                    $metrics.UniqueErrors++
                }

                # PERFORMANCE: Use compiled regex instead of inline -match
                if ($this.AccessDeniedRegex.IsMatch($key)) {
    ...
    # ENHANCEMENT: Validate analytics result integrity
    hidden [void] ValidateAnalyticsResult([AnalyticsResult]$result) {
        if ($null -eq $result) {
            throw "AnalyticsResult cannot be null"
        }
        ...
        if ($null -eq $result.RiskAssessment) {
        ...
        # Validate data types
        if ($result.HealthScore -isnot [double]) {
            $result.HealthScore = [double]$result.HealthScore
        }

        if ($result.Anomalies.Count -isnot [int]) {
            $result.Anomalies.Count = [int]$result.Anomalies.Count
        }
    }
}
