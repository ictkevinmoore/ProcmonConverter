...
.EXAMPLEcd .\Ultimate-Modular-ProcmonAnalysis-Suite-INTEGRATED.ps1 -InputDirectory "C:\Users\ictke\OneDrive\Desktop\ProcmonConverter\Data\Converted"
...
# Load StreamingCSVProcessor using alternative method for classes
$streamingProcessorPath = Join-Path $Script:ScriptRoot "StreamingCSVProcessor.ps1"
if (Test-Path $streamingProcessorPath) {
    # Execute the script in the current scope to load the class
    $streamingProcessorScript = Get-Content $streamingProcessorPath -Raw
    Invoke-Expression $streamingProcessorScript

    # Verify class is loaded
    try {
        $testInstance = [StreamingCSVProcessor]::new(1000, $false)
        $testInstance = $null
        Write-Host "  ✓ StreamingCSVProcessor class loaded successfully" -ForegroundColor Green
    }
    catch {
        Write-Error "Failed to load StreamingCSVProcessor class: $_"
    }
} else {
...
function Invoke-IntegratedProcmonAnalysis {
    ...
    try {
        ...
        Write-Host "║  Error: $($errorMessage.PadRight(60)) ║" -ForegroundColor Red
        Write-Host "╚══════════════════════════════════════════════════════════════════════╝`n" -ForegroundColor Red
