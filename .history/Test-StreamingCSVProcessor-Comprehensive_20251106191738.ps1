...
function Test-ScriptSyntax {
    ...
    try {
        ...
        Write-TestResult `
            ...
            -Passed $passed `
            -Message $(if ($parseErrors.Count -gt 0) { "$($parseErrors.Count) syntax errors found" } else { "No syntax errors" })

        if ($parseErrors.Count -gt 0) {
            foreach ($parseError in $parseErrors) {
                Write-TestWarning "Line $($parseError.Token.StartLine): $($parseError.Message)"
            }
        }
