...
class ReportValidator {
    ...
    [ValidationResult]ValidateAll([hashtable]$dataObject, [hashtable]$sessionInfo, [string]$outputPath, [hashtable]$config) {
        ...
        if (-not $dataResult.IsValid) {
            foreach ($validationError in $dataResult.Errors) {
                $result.AddError("DataObject: $validationError")
            }
        }
        ...
        if (-not $sessionResult.IsValid) {
            foreach ($validationError in $sessionResult.Errors) {
                $result.AddError("SessionInfo: $validationError")
            }
        }
        ...
        if (-not $pathResult.IsValid) {
            foreach ($validationError in $pathResult.Errors) {
                $result.AddError("OutputPath: $validationError")
            }
        }
        ...
        if (-not $configResult.IsValid) {
            foreach ($validationError in $configResult.Errors) {
                $result.AddError("Configuration: $validationError")
            }
        }
