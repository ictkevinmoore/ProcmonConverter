...
try {
    ...
    # Robust error handling - handle cases where exception details may be missing
    ...
    try {
        ...
        elseif ($_ -and $_.InvocationInfo) {
            $stackTrace = $_.InvocationInfo.PositionMessage
        }
    }
    catch {
        # If we can't even get error details, use a generic message
