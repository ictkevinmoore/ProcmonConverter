...
function New-ReportHTML {
    ...
    try {
        ...
        $tabContent = $this.GenerateTabContent($ReportData, $SessionInfo)
        $htmlBuilder.AppendLine($tabContent) | Out-Null
        ...
        $htmlBuilder.AppendLine('                select: {') | Out-Null
        $htmlBuilder.AppendLine('                    style: "multi",') | Out-Null
        $htmlBuilder.AppendLine('                    selector: "td:first-child"') | Out-Null
        $htmlBuilder.AppendLine('                },') | Out-Null
        ...
        $htmlBuilder.AppendFormat('                        text: "<i class=\"fas fa-file-excel\"></i> Excel ({0} selected)",') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendFormat('                        title: "Procmon Analysis - {0} - " + new Date().toISOString().split("T")[0],', $SessionInfo.SessionId) | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('                        exportOptions: {') | Out-Null
        $htmlBuilder.AppendLine('                            modifier: { selected: true },') | Out-Null
        $htmlBuilder.AppendLine('                            orthogonal: "export"') | Out-Null
        $htmlBuilder.AppendLine('                        }') | Out-Null
        $htmlBuilder.AppendLine('                    },') | Out-Null
        $htmlBuilder.AppendLine('                    {') | Out-Null
        $htmlBuilder.AppendLine('                        extend: "csv",') | Out-Null
        $htmlBuilder.AppendFormat('                        text: "<i class=\"fas fa-file-csv\"></i> CSV ({0} selected)",') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendFormat('                        title: "Procmon Analysis - {0} - " + new Date().toISOString().split("T")[0],', $SessionInfo.SessionId) | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('                        exportOptions: {') | Out-Null
        ...
        $htmlBuilder.AppendFormat('                        text: "<i class=\"fas fa-file-pdf\"></i> PDF ({0} selected)",') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendFormat('                        title: "Procmon Analysis - {0}",', $SessionInfo.SessionId) | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        ...
        $htmlBuilder.AppendFormat('                        text: "<i class=\"fas fa-copy\"></i> Copy ({0} selected)",') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('                        exportOptions: {') | Out-Null
        $htmlBuilder.AppendLine('                            modifier: { selected: true },') | Out-Null
        $htmlBuilder.AppendLine('                            orthogonal: "export"') | Out-Null
        ...
        $htmlBuilder.AppendFormat('                        text: "<i class=\"fas fa-print\"></i> Print ({0} selected)",') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('                        exportOptions: {') | Out-Null
        $htmlBuilder.AppendLine('                            modifier: { selected: true },') | Out-Null
        $htmlBuilder.AppendLine('                            orthogonal: "export"') | Out-Null
        ...
        # Initialize thumbnail charts
        $htmlBuilder.AppendLine('            // Initialize thumbnail charts on page load') | Out-Null
        $htmlBuilder.AppendFormat('            const processThumbnailCanvas = document.getElementById("processThumbnail");') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('            if (processThumbnailCanvas) {') | Out-Null
        $htmlBuilder.AppendFormat('                const labels = processThumbnailCanvas.dataset.labels.split(",");') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendFormat('                const data = processThumbnailCanvas.dataset.data.split(",").map(Number);') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        ...
        $htmlBuilder.AppendLine('            }') | Out-Null
        $htmlBuilder.AppendFormat('            const operationThumbnailCanvas = document.getElementById("operationThumbnail");') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('            if (operationThumbnailCanvas) {') | Out-Null
        $htmlBuilder.AppendFormat('                const labels = operationThumbnailCanvas.dataset.labels.split(",");') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendFormat('                const data = operationThumbnailCanvas.dataset.data.split(",").map(Number);') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        ...
        $htmlBuilder.AppendFormat('                    const labels = canvas.dataset.labels.split(",");') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendFormat('                    const data = canvas.dataset.data.split(",").map(Number);') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
