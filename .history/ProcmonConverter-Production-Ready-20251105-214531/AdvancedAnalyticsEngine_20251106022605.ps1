...
class AdvancedAnalyticsEngine {
    ...
    [AnalyticsResult] AnalyzeData([hashtable]$processedData) {
        ...
        if ($this.EnableCaching -and $this.MetricsCache.ContainsKey($metricsKey)) {
            ...
            $result.Metrics = $this.MetricsCalc.CalculateMetricsOptimized($processedData)
        ...
        if ($processedData.ContainsKey('Statistics') -and $processedData.Statistics.ProcessTypes) {
            ...
            if ($this.EnableCaching -and $this.AnomalyCache.ContainsKey($anomalyKey)) {
                ...
                $result.Anomalies = $this.StatAnalyzer.DetectAnomaliesOptimized($processedData.Statistics.ProcessTypes)
    ...
    [hashtable] CalculateMetricsOptimized([hashtable]$processedData) {
        ...
        # OPTIMIZATION: Use TryGetValue for better performance
        [int]$recordCount = 0
        if ($processedData.TryGetValue('RecordCount', [ref]$recordCount)) {
            $metrics.TotalEvents = $recordCount
        }
        ...
        $stats = $null
        if (-not $processedData.TryGetValue('Statistics', [ref]$stats) -or -not $stats) {
            return $metrics
        }

        # OPTIMIZATION: Calculate rates with single pass
        $results = $null
        if ($stats.TryGetValue('Results', [ref]$results) -and $results) {
            $totalResults = 0
            $errorsCount = 0
            $accessDeniedCount = 0
        ...
        $processTypes = $null
        if ($stats.TryGetValue('ProcessTypes', [ref]$processTypes) -and $processTypes) {
        ...
        $operations = $null
        if ($stats.TryGetValue('Operations', [ref]$operations) -and $operations) {
        ...
        $performance = $null
        if ($processedData.TryGetValue('Performance', [ref]$performance) -and $performance) {
            $durationSeconds = 0
            if ($performance.TryGetValue('DurationSeconds', [ref]$durationSeconds) -and $durationSeconds -gt 0) {
                $metrics.EventsPerSecond = [Math]::Round($metrics.TotalEvents / $durationSeconds, 2)
            }
        }
