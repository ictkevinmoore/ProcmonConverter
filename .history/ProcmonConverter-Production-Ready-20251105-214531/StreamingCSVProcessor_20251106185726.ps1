...
class StreamingCSVProcessor {
    ...
    [hashtable] ProcessFile([string]$filePath, [scriptblock]$recordFilter, [bool]$enablePostProcessing, [CSVPostProcessingOptions]$postProcessingOptions) {
        ...
        try {
            ...
            if ([string]::IsNullOrWhiteSpace($filePath)) {
                throw [ArgumentException]::new("File path cannot be null or empty", "filePath")
            }
            ...
            # ENHANCEMENT: Additional validation
            $fileInfo = Get-Item $filePath -ErrorAction Stop
            if ($fileInfo.Length -eq 0) {
                Write-Warning "File is empty: $filePath"
            }
            if ($fileInfo.Extension -ne '.csv') {
                Write-Warning "File does not have .csv extension: $filePath"
            }
    ...
    # OPTIMIZATION: Parse CSV line with compiled regex for better performance
    hidden [string[]] ParseCSVLineOptimized([string]$line) {
        # OPTIMIZATION: Use compiled regex for faster parsing
        $matches = $this.CompiledRegex.Matches($line)
        $values = [List[string]]::new()
        $startIndex = 0

        foreach ($match in $matches) {
            $value = $line.Substring($startIndex, $match.Index - $startIndex)
    ...
    # OPTIMIZATION: Process batch with caching and optimized statistics aggregation
    hidden [void] ProcessBatchOptimized([List[hashtable]]$batch) {
        foreach ($record in $batch) {
            $this.AggregateStatisticsOptimized($record)
        }

        $this.TotalRecordsProcessed += $batch.Count
        $this.BatchesProcessed++

        # OPTIMIZATION: Cache batch statistics for faster access
        if ($this.EnableCaching -and $this.ResultCache.Count -lt $this.CacheSize) {
            $batchKey = "batch_$($this.BatchesProcessed)"
            $this.ResultCache[$batchKey] = @{
                Count = $batch.Count
    ...
    # OPTIMIZATION: Aggregate statistics with caching and optimized lookups
    hidden [void] AggregateStatisticsOptimized([hashtable]$record) {
        # OPTIMIZATION: Use cached lookups and avoid repeated ContainsKey calls
        $procName = $record['Process Name']
        if (-not [string]::IsNullOrEmpty($procName)) {
            # OPTIMIZATION: Use TryGetValue for faster dictionary access
            $currentCount = 0
            if ($this.ProcessTypes.TryGetValue($procName, [ref]$currentCount)) {
                $this.ProcessTypes[$procName] = $currentCount + 1
            } else {
                $this.ProcessTypes[$procName] = 1
            }
        }
        ...
        if (-not [string]::IsNullOrEmpty($op)) {
            $currentCount = 0
            if ($this.Operations.TryGetValue($op, [ref]$currentCount)) {
                $this.Operations[$op] = $currentCount + 1
            } else {
                $this.Operations[$op] = 1
            }
        }
        ...
        if (-not [string]::IsNullOrEmpty($result)) {
            $currentCount = 0
            if ($this.Results.TryGetValue($result, [ref]$currentCount)) {
                $this.Results[$result] = $currentCount + 1
            } else {
                $this.Results[$result] = 1
            }
        }
