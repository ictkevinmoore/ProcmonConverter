...
function New-ReportHTML {
    ...
    try {
        ...
        $totalEventsDetails = "Total Events|$($analytics.Metrics.TotalEvents:N0) events processed|Analysis Period: Full Dataset|Processing Status: Complete"
        $htmlBuilder.AppendFormat('                                <div class="col-md-3"><div class="metric-card insight-clickable" data-insight-type="metric-total" data-insight-details="{0}"><div class="value">{1}</div><div class="label">Total Events</div></div></div>', $totalEventsDetails, $analytics.Metrics.TotalEvents) | Out-Null
