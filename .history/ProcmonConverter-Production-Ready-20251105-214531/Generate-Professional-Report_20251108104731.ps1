...
.NOTES
    ...
    Refactoring: Modular architecture with separate concerns, externalized templates
#>
...
Import-Module (Join-Path $modulesPath "ReportConfiguration.psm1") -Force -ErrorAction Stop
Import-Module (Join-Path $modulesPath "ReportLogger.psm1") -Force -ErrorAction Stop
Import-Module (Join-Path $modulesPath "ReportValidation.psm1") -Force -ErrorAction Stop
...
function Get-TemplatePath {
    ...
    $scriptPath = Split-Path -Parent $MyInvocation.MyCommand.Path
    $templatePath = Join-Path $scriptPath "Templates\$TemplateName"
...
function Format-RecommendationsList {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
    ...
    foreach ($recommendation in $Recommendations) {
        $encodedRec = [System.Web.HttpUtility]::HtmlEncode($recommendation)
        $html += "<li class=`"list-group-item`">$encodedRec</li>"
    }
    return $html
}

function Format-PatternsList {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [array]$Patterns
    ...
    foreach ($pattern in $Patterns) {
        ...
        $encodedType = [System.Web.HttpUtility]::HtmlEncode($pattern.Type)
        $encodedDesc = [System.Web.HttpUtility]::HtmlEncode($pattern.Description)
        $html += "<div class=`"col-md-6 mb-3`"><div class=`"alert alert-$badgeClass`"><strong>$encodedType:</strong> $encodedDesc<span class=`"badge bg-$badgeClass float-end`">$($pattern.Severity)</span></div></div>"
    }
    return $html
}

function Format-ProcessClusters {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [array]$ProcessClusters
    )

    $html = ""
    foreach ($cluster in $ProcessClusters) {
        $encodedCategory = [System.Web.HttpUtility]::HtmlEncode($cluster.Characteristics.Category)
        $html += "<div class=`"col-md-4 mb-3`"><div class=`"metric-card`"><div class=`"label`">$encodedCategory Activity Processes</div><div class=`"value`">$($cluster.Processes.Count)</div></div></div>"
    }
    return $html
}

function Format-AnomaliesList {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [object]$Analytics
    ...
    if ($Analytics.Anomalies.Count -gt 0) {
        ...
        foreach ($anomaly in $Analytics.Anomalies.Items) {
            $encodedAnomaly = [System.Web.HttpUtility]::HtmlEncode($anomaly)
            $html += "<li class=`"list-group-item`">$encodedAnomaly</li>"
        }
        $html += "</ul>"
    } else {
...
function Format-TableHeaders {
    ...
    $html = "<th>#</th>"
    foreach ($prop in $SampleEvents[0].PSObject.Properties.Name) {
        $encodedProp = [System.Web.HttpUtility]::HtmlEncode($prop)
        $html += "<th>$encodedProp</th>"
    }
    return $html
}
