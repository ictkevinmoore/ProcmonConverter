...
function New-ReportHTML {
    ...
    try {
        ...
        $htmlBuilder.AppendLine('            </li>') | Out-Null
        $htmlBuilder.AppendLine('            <li class="nav-item" role="presentation">') | Out-Null
        $htmlBuilder.AppendLine('                <button class="nav-link" id="tab-ml-btn" data-bs-toggle="tab" data-bs-target="#tab-ml" type="button" role="tab">') | Out-Null
        $htmlBuilder.AppendLine('                    <i class="fas fa-robot me-2"></i>ML Analytics') | Out-Null
        $htmlBuilder.AppendLine('                </button>') | Out-Null
        $htmlBuilder.AppendLine('            </li>') | Out-Null
        $htmlBuilder.AppendLine('            <li class="nav-item" role="presentation">') | Out-Null
        ...
        $htmlBuilder.AppendLine('            <!-- Tab 1: Executive Summary -->') | Out-Null
        ...
        if ($analytics.Anomalies.Count -gt 0) {
            ...
            foreach ($anomaly in $analytics.Anomalies.Items) {
                $anomalyDetails = "Anomaly Detail|$([System.Web.HttpUtility]::HtmlEncode($anomaly))|Detection Method: Statistical Analysis|Severity: High|Impact: Requires immediate investigation"
                $htmlBuilder.AppendFormat('                                <li class="list-group-item insight-clickable" data-insight-type="anomaly" data-insight-details="{0}"><i class="fas fa-exclamation-circle text-danger me-2"></i>{1}</li>', $anomalyDetails, [System.Web.HttpUtility]::HtmlEncode($anomaly)) | Out-Null
            }
            $htmlBuilder.AppendLine('                            </ul>') | Out-Null
        } else {
        ...
        # TAB 4: ML ANALYTICS (Separate Tab) - Make items clickable
        ...
        # Build trend details for modal
        $trendDetails = "Trend Direction: $($patterns.TemporalPatterns.TrendDirection)|Analysis Method: Time-series ML|Impact: Indicates system activity trajectory|Recommendation: Monitor for unexpected changes"
        $htmlBuilder.AppendFormat('                                    <div class="alert alert-info insight-clickable" data-insight-type="ml-trend" data-insight-details="{0}">', $trendDetails) | Out-Null
        ...
        $htmlBuilder.AppendLine('                                </div>') | Out-Null
        $htmlBuilder.AppendLine('                                <div class="col-md-6">') | Out-Null
        # Build seasonality details for modal
        $seasonalityDetails = "Seasonality Pattern: $($patterns.TemporalPatterns.Seasonality)|Detection Method: ML Pattern Recognition|Frequency: Recurring temporal patterns|Predictive Value: Helps forecast future behavior"
        $htmlBuilder.AppendFormat('                                    <div class="alert alert-primary insight-clickable" data-insight-type="ml-seasonality" data-insight-details="{0}">', $seasonalityDetails) | Out-Null
        ...
        # Build risk level details for modal
        $riskLevelDetails = "Risk Level: $($analytics.RiskAssessment.Level)|Risk Score: $($analytics.RiskAssessment.Total)/100|Assessment Method: Comprehensive ML Analysis|Action Required: " + $(if($analytics.RiskAssessment.Level -eq "High"){"Immediate investigation"}else{"Continue monitoring"})
        $htmlBuilder.AppendFormat('                                    <div class="metric-card insight-clickable" data-insight-type="ml-risk-level" data-insight-details="{0}">', $riskLevelDetails) | Out-Null
        ...
        $htmlBuilder.AppendLine('                                </div>') | Out-Null
        $htmlBuilder.AppendLine('                                <div class="col-md-4">') | Out-Null
        # Build risk score details for modal
        $riskScoreDetails = "Risk Score: $($analytics.RiskAssessment.Total)/100|Calculation Method: Multi-factor ML assessment|Factors: Error rate, anomalies, patterns|Threshold: >70 requires attention"
        $htmlBuilder.AppendFormat('                                    <div class="metric-card insight-clickable" data-insight-type="ml-risk-score" data-insight-details="{0}">', $riskScoreDetails) | Out-Null
        ...
        $htmlBuilder.AppendLine('                                </div>') | Out-Null
        $htmlBuilder.AppendLine('                                <div class="col-md-4">') | Out-Null
        # Build system health details for modal
        $systemHealthDetails = "System Health: $($analytics.HealthScore.ToString('N1'))/100|Status: " + $(if($analytics.HealthScore -gt 80){"Excellent"}elseif($analytics.HealthScore -gt 60){"Good"}else{"Needs Improvement"})|Based On: $($analytics.Metrics.TotalEvents) events analyzed|Confidence Level: High"
        $htmlBuilder.AppendFormat('                                    <div class="metric-card insight-clickable" data-insight-type="ml-health" data-insight-details="{0}">', $systemHealthDetails) | Out-Null
        ...
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('        <!-- Insight Detail Modal -->') | Out-Null
        $htmlBuilder.AppendLine('        <div class="modal fade insight-modal" id="insightDetailModal" tabindex="-1">') | Out-Null
        $htmlBuilder.AppendLine('            <div class="modal-dialog modal-xl">') | Out-Null
        $htmlBuilder.AppendLine('                <div class="modal-content">') | Out-Null
        $htmlBuilder.AppendLine('                    <div class="modal-header">') | Out-Null
        $htmlBuilder.AppendLine('                        <h5 class="modal-title" id="insightModalTitle"><i class="fas fa-lightbulb"></i> AI-Powered Insight Details</h5>') | Out-Null
        $htmlBuilder.AppendLine('                        <button type="button" class="btn btn-sm btn-outline-secondary copy-btn" id="copyInsightBtn" title="Copy to clipboard">') | Out-Null
        $htmlBuilder.AppendLine('                            <i class="fas fa-copy"></i> Copy') | Out-Null
        $htmlBuilder.AppendLine('                        </button>') | Out-Null
        $htmlBuilder.AppendLine('                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>') | Out-Null
        $htmlBuilder.AppendLine('                    </div>') | Out-Null
        $htmlBuilder.AppendLine('                    <div class="modal-body">') | Out-Null
        $htmlBuilder.AppendLine('                        <div id="insightDetailContent">') | Out-Null
        $htmlBuilder.AppendLine('                            <!-- Dynamic insight details will be inserted here -->') | Out-Null
        $htmlBuilder.AppendLine('                        </div>') | Out-Null
        $htmlBuilder.AppendLine('                    </div>') | Out-Null
        $htmlBuilder.AppendLine('                    <div class="modal-footer">') | Out-Null
        $htmlBuilder.AppendLine('                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>') | Out-Null
        ...
        $htmlBuilder.AppendLine('            });') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('            // Clickable Insight Handler') | Out-Null
        $htmlBuilder.AppendLine('            $(document).on("click", ".insight-clickable", function(e) {') | Out-Null
        $htmlBuilder.AppendLine('                e.stopPropagation();') | Out-Null
        $htmlBuilder.AppendLine('                const insightType = $(this).data("insight-type");') | Out-Null
        $htmlBuilder.AppendLine('                const insightDetails = $(this).data("insight-details");') | Out-Null
        $htmlBuilder.AppendLine('                ') | Out-Null
        $htmlBuilder.AppendLine('                if (insightDetails) {') | Out-Null
        $htmlBuilder.AppendLine('                    showInsightDetails(insightType, insightDetails);') | Out-Null
        $htmlBuilder.AppendLine('                }') | Out-Null
        $htmlBuilder.AppendLine('            });') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('            // Function to parse and display insight details') | Out-Null
        $htmlBuilder.AppendLine('            function showInsightDetails(type, details) {') | Out-Null
        $htmlBuilder.AppendLine('                const parts = details.split("|");') | Out-Null
        $htmlBuilder.AppendLine('                let contentHtml = "";') | Out-Null
        $htmlBuilder.AppendLine('                ') | Out-Null
        $htmlBuilder.AppendLine('                // Create header with icon based on type') | Out-Null
        $htmlBuilder.AppendLine('                const iconMap = {') | Out-Null
        $htmlBuilder.AppendLine('                    "health-score": "fa-heart-pulse",') | Out-Null
        $htmlBuilder.AppendLine('                    "key-insight": "fa-lightbulb",') | Out-Null
        $htmlBuilder.AppendLine('                    "recommendation": "fa-tasks",') | Out-Null
        $htmlBuilder.AppendLine('                    "pattern": "fa-brain",') | Out-Null
        $htmlBuilder.AppendLine('                    "cluster": "fa-object-group",') | Out-Null
        $htmlBuilder.AppendLine('                    "metric-total": "fa-database",') | Out-Null
        $htmlBuilder.AppendLine('                    "metric-errors": "fa-exclamation-triangle",') | Out-Null
        $htmlBuilder.AppendLine('                    "metric-anomalies": "fa-search",') | Out-Null
        $htmlBuilder.AppendLine('                    "metric-risk": "fa-shield-alt",') | Out-Null
        $htmlBuilder.AppendLine('                    "anomaly": "fa-exclamation-circle",') | Out-Null
        $htmlBuilder.AppendLine('                    "ml-trend": "fa-chart-line",') | Out-Null
        $htmlBuilder.AppendLine('                    "ml-seasonality": "fa-clock",') | Out-Null
        $htmlBuilder.AppendLine('                    "ml-risk-level": "fa-shield-alt",') | Out-Null
        $htmlBuilder.AppendLine('                    "ml-risk-score": "fa-tachometer-alt",') | Out-Null
        $htmlBuilder.AppendLine('                    "ml-health": "fa-heartbeat"') | Out-Null
        $htmlBuilder.AppendLine('                };') | Out-Null
        $htmlBuilder.AppendLine('                ') | Out-Null
        $htmlBuilder.AppendLine('                const icon = iconMap[type] || "fa-info-circle";') | Out-Null
        $htmlBuilder.AppendLine('                ') | Out-Null
        $htmlBuilder.AppendLine('                contentHtml += "<div class=\"insight-detail-header\">";') | Out-Null
        $htmlBuilder.AppendLine('                contentHtml += "  <i class=\"fas " + icon + "\"></i>";') | Out-Null
        $htmlBuilder.AppendLine('                contentHtml += "  <div>";') | Out-Null
        $htmlBuilder.AppendLine('                contentHtml += "    <h4>" + parts[0] + "</h4>";') | Out-Null
        $htmlBuilder.AppendLine('                contentHtml += "    <p class=\"text-muted mb-0\">AI-Powered Analysis Result</p>";') | Out-Null
        $htmlBuilder.AppendLine('                contentHtml += "  </div>";') | Out-Null
        $htmlBuilder.AppendLine('                contentHtml += "</div>";') | Out-Null
        $htmlBuilder.AppendLine('                ') | Out-Null
        $htmlBuilder.AppendLine('                // Display remaining parts as detail sections') | Out-Null
        $htmlBuilder.AppendLine('                if (parts.length > 1) {') | Out-Null
        $htmlBuilder.AppendLine('                    contentHtml += "<div class=\"insight-detail-section\">";') | Out-Null
        $htmlBuilder.AppendLine('                    contentHtml += "  <h6>Details</h6>";') | Out-Null
        $htmlBuilder.AppendLine('                    contentHtml += "  <div class=\"insight-metric-grid\">";') | Out-Null
        $htmlBuilder.AppendLine('                    ') | Out-Null
        $htmlBuilder.AppendLine('                    for (let i = 1; i < parts.length; i++) {') | Out-Null
        $htmlBuilder.AppendLine('                        const detail = parts[i];') | Out-Null
        $htmlBuilder.AppendLine('                        const colonIndex = detail.indexOf(":");') | Out-Null
        $htmlBuilder.AppendLine('                        ') | Out-Null
        $htmlBuilder.AppendLine('                        if (colonIndex > 0) {') | Out-Null
        $htmlBuilder.AppendLine('                            const label = detail.substring(0, colonIndex).trim();') | Out-Null
        $htmlBuilder.AppendLine('                            const value = detail.substring(colonIndex + 1).trim();') | Out-Null
        $htmlBuilder.AppendLine('                            ') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "    <div class=\"insight-metric-item\">";') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "      <div class=\"value\">" + value + "</div>";') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "      <div class=\"label\">" + label + "</div>";') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "    </div>";') | Out-Null
        $htmlBuilder.AppendLine('                        } else {') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "    <div class=\"insight-metric-item\">";') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "      <div class=\"value\">" + detail + "</div>";') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "      <div class=\"label\">Information</div>";') | Out-Null
        $htmlBuilder.AppendLine('                            contentHtml += "    </div>";') | Out-Null
        $htmlBuilder.AppendLine('                        }') | Out-Null
        $htmlBuilder.AppendLine('                    }') | Out-Null
        $htmlBuilder.AppendLine('                    ') | Out-Null
        ...
        $htmlBuilder.AppendLine('                }') | Out-Null
        $htmlBuilder.AppendLine('                ') | Out-Null
        $htmlBuilder.AppendLine('                $("#insightDetailContent").html(contentHtml);') | Out-Null
        $htmlBuilder.AppendLine('                $("#insightDetailModal").modal("show");') | Out-Null
        $htmlBuilder.AppendLine('            }') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('            // Copy insight to clipboard') | Out-Null
        $htmlBuilder.AppendLine('            $("#copyInsightBtn").on("click", function() {') | Out-Null
        $htmlBuilder.AppendLine('                const content = $("#insightDetailContent").text();') | Out-Null
        $htmlBuilder.AppendLine('                navigator.clipboard.writeText(content).then(() => {') | Out-Null
        $htmlBuilder.AppendLine('                    const btn = $(this);') | Out-Null
        $htmlBuilder.AppendLine('                    const originalHtml = btn.html();') | Out-Null
        $htmlBuilder.AppendLine('                    btn.html("<i class=\"fas fa-check\"></i> Copied!");') | Out-Null
        $htmlBuilder.AppendLine('                    btn.removeClass("btn-outline-secondary").addClass("btn-success");') | Out-Null
        $htmlBuilder.AppendLine('                    ') | Out-Null
        $htmlBuilder.AppendLine('                    setTimeout(() => {') | Out-Null
        $htmlBuilder.AppendLine('                        btn.html(originalHtml);') | Out-Null
        $htmlBuilder.AppendLine('                        btn.removeClass("btn-success").addClass("btn-outline-secondary");') | Out-Null
        $htmlBuilder.AppendLine('                    }, 2000);') | Out-Null
        $htmlBuilder.AppendLine('                }).catch(err => {') | Out-Null
        $htmlBuilder.AppendLine('                    console.error("Failed to copy:", err);') | Out-Null
        $htmlBuilder.AppendLine('                    alert("Failed to copy to clipboard");') | Out-Null
        $htmlBuilder.AppendLine('                });') | Out-Null
        $htmlBuilder.AppendLine('            });') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('            // Keyboard navigation for insight modal') | Out-Null
        $htmlBuilder.AppendLine('            $("#insightDetailModal").on("shown.bs.modal", function() {') | Out-Null
        $htmlBuilder.AppendLine('                $(document).on("keydown.insightModal", function(e) {') | Out-Null
        $htmlBuilder.AppendLine('                    if (e.key === "Escape") {') | Out-Null
        $htmlBuilder.AppendLine('                        $("#insightDetailModal").modal("hide");') | Out-Null
        $htmlBuilder.AppendLine('                    }') | Out-Null
        $htmlBuilder.AppendLine('                });') | Out-Null
        $htmlBuilder.AppendLine('            });') | Out-Null
        $htmlBuilder.AppendLine() | Out-Null
        $htmlBuilder.AppendLine('            $("#insightDetailModal").on("hidden.bs.modal", function() {') | Out-Null
        $htmlBuilder.AppendLine('                $(document).off("keydown.insightModal");') | Out-Null
        $htmlBuilder.AppendLine('            });') | Out-Null
        $htmlBuilder.AppendLine('        });') | Out-Null
        $htmlBuilder.AppendLine('    </script>') | Out-Null
